...
public class _03_MurderOfCrows {
	...
	private void findTheDiamond() {
		...
		for (int i = 0; i < theMurder.size(); i++) {
			Crow search = theMurder.get(i);
			ArrayList<String> searchstomach = search.getStomachContents();
			for (int j = 0; j < searchstomach.size(); j++) {
				if (searchstomach.get(j).equals("diamond")) {
					JOptionPane.showMessageDialog(null, "" + search.getName()+" had the diamond, "+(i+1)+" died before you found the dimond");
